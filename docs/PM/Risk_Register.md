...
| OCLW-RSK-0016 | Uso de plugin dinamico (`dlopen`/`dlsym`) mantiene superficie de ataque en RT (path injection/sustitucion de backend) | Media | Alta | En RT: usar ruta de solo lectura y whitelist, control CM del `.so`, deshabilitar env vars en modo estricto, validar owner/permisos; opcional hash/firma del plugin antes de cargar | <TBD_OWNER> | Abierto |
| OCLW-RSK-0017 | Plugin/verificador de referencia permanece `NOT CRYPTO` y no es apto para autenticidad de produccion | Media | Alta | Sustituir por proveedor criptografico aprobado (PKI/HSM/libreria certificada) y mantener fail-closed si no esta disponible o no valida firma | <TBD_OWNER> | Abierto |
