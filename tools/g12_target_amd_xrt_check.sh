...
REPORT_DIR="${REPO_ROOT}/docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01"
mkdir -p "${REPORT_DIR}"
...
if [ -d /etc/OpenCL/vendors ]; then
  if ls /etc/OpenCL/vendors/*.icd >/dev/null 2>&1; then
    if grep -Eiq 'xilinx|xrt|amd|rocm' /etc/OpenCL/vendors/*.icd 2>/dev/null; then
      contains_xrt_icd=1
    fi
    if ls /etc/OpenCL/vendors/*.icd 2>/dev/null | grep -Eiq 'xilinx|amd|rocm'; then
      contains_xrt_icd=1
    fi
  fi
fi
...
has_xrt_runtime=0
if ldconfig -p 2>/dev/null | grep -Eiq 'xrt_core|xilinxopencl|amdocl|rocm'; then
  has_xrt_runtime=1
fi

has_xrt_tools=0
if command -v xbutil >/dev/null 2>&1 || command -v xbutil2 >/dev/null 2>&1 || command -v xrt-smi >/dev/null 2>&1 || command -v rocm-smi >/dev/null 2>&1; then
  has_xrt_tools=1
fi

has_xrt_env=0
if [ -n "${XILINX_XRT:-}" ] || [ -n "${XRT_ROOT:-}" ] || [ -n "${XILINX_VITIS:-}" ]; then
  has_xrt_env=1
fi

{
  echo "stack=AMD_XRT"
  echo "contains_xrt_icd=${contains_xrt_icd}"
  echo "has_xrt_runtime=${has_xrt_runtime}"
  echo "has_xrt_tools=${has_xrt_tools}"
  echo "has_xrt_env=${has_xrt_env}"
  echo "XILINX_XRT=${XILINX_XRT:-<unset>}"
  echo "XRT_ROOT=${XRT_ROOT:-<unset>}"
  echo "XILINX_VITIS=${XILINX_VITIS:-<unset>}"
  echo "XCL_EMULATION_MODE=${XCL_EMULATION_MODE:-<unset>}"
  ...
  ldconfig -p 2>/dev/null | grep -Ei 'opencl|xrt|xilinx|amd|rocm' || true
  echo "ldconfig_opencl_end"
} >"${DETECT_LOG}" 2>&1

stack_present=0
if { [ "${contains_xrt_icd}" -eq 1 ] || [ "${has_xrt_runtime}" -eq 1 ]; } && { [ "${has_xrt_tools}" -eq 1 ] || [ "${has_xrt_env}" -eq 1 ]; }; then
  stack_present=1
fi
...
# G12 AMD/XRT Target Acceptance Report
...
## Detection Summary
- contains_xrt_icd=${contains_xrt_icd}
- has_xrt_runtime=${has_xrt_runtime}
- has_xrt_tools=${has_xrt_tools}
- has_xrt_env=${has_xrt_env}
- XCL_EMULATION_MODE=${XCL_EMULATION_MODE:-<unset>}

## Evidence
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/00_context.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/01_detection.log
...
## Evidence
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/02_preflight.log
...
PACK_BASE="/tmp/oclw_g12_amd_xrt_${TS_COMPACT}"
mkdir -p "${PACK_BASE}/strict" "${PACK_BASE}/bench"
POCL_CACHE_DIR="/tmp/oclw_g12_amd_xrt_pocl_${TS_COMPACT}"
...
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/01_detection.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/02_preflight.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/03_plugin.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/10_smoke_platforms.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/11_smoke_rt_strict_policy.log
- docs/VV/Execution_Logs/GATES/G12_AMD_XRT/rerun_01/12_bench_rt_pack_add1.log
