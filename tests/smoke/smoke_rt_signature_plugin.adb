...
procedure Smoke_RT_Signature_Plugin is
   ...
   Default_Pack_Dir : constant String :=
     "docs/VV/Execution_Logs/local/pack_sig_plugin";
   ...
   function Tamper_Signature (Value : String) return String is
      Result : String := Value;
   begin
...
begin
   declare
      ...
      Meta_Pos : Packs.Pack_Metadata;
      Meta_Neg : Packs.Pack_Metadata;
   ...
   begin
      ...
      Ada.Environment_Variables.Set ("OCLW_CRYPTO_PLUGIN", Packs.To_String (Plugin_Path));
      Ada.Environment_Variables.Set ("OCLW_CRYPTO_SYMBOL", Plugin_Symbol);
      Ada.Text_IO.Put_Line ("INFO oclw_crypto_plugin=" & Packs.To_String (Plugin_Path));
      Ada.Text_IO.Put_Line ("INFO oclw_crypto_symbol=" & Plugin_Symbol);
      ...
      --  Positive path: plugin verifies valid TEST-FNV1A32 signature.
      Meta_Pos := Meta_Base;
      Meta_Pos.Signature_Required := True;
      Meta_Pos.Signature_Alg := Packs.To_Bounded ("TEST-FNV1A32");
      Meta_Pos.Signer_Id := Packs.To_Bounded ("OCLW-PLUGIN-REF");
      Meta_Pos.Signature_Value := Packs.To_Bounded ("");

      declare
         Signing_Text : constant String := Packs.Canonical_Signing_Text (Meta_Pos);
      ...
      begin
         if Signing_Text'Length = 0 or else Signature'Length = 0 then
            Status := Errors.OCLW_Pack_Format_Error;
            Ada.Text_IO.Put_Line
              ("INFO positive_case=FAIL signing_text/signature_invalid status_int="
               & Status_Int_Image (Status));
         else
            Meta_Pos.Signature_Value := Packs.To_Bounded (Signature);
            ...
            if Status /= Errors.Success then
               Ada.Text_IO.Put_Line
                 ("INFO positive_case=FAIL write_manifest_status="
                  & Errors.Image (Status)
                  & " status_int="
                  & Status_Int_Image (Status));
            else
               ...
               if Status = Errors.Success then
                  Positive_Pass := True;
                  Ada.Text_IO.Put_Line ("INFO positive_case=PASS");
               else
                  Ada.Text_IO.Put_Line
                    ("INFO positive_case=FAIL status="
                     & Errors.Image (Status)
                     & " status_int="
                     & Status_Int_Image (Status));
               end if;
      ...
      --  Negative path: tampered signature must fail closed.
      Meta_Neg := Meta_Pos;
      Meta_Neg.Signature_Value :=
        Packs.To_Bounded
          (Tamper_Signature (Packs.To_String (Meta_Pos.Signature_Value)));
      ...
      if Status /= Errors.Success then
         Ada.Text_IO.Put_Line
           ("INFO negative_case=FAIL write_manifest_status="
            & Errors.Image (Status)
            & " status_int="
            & Status_Int_Image (Status));
      else
         Execute_Add1
           (Meta => Meta_Neg,
            ...
            Used => Binary_Used,
            Status => Status);
         if Status = Errors.OCLW_Signature_Invalid then
            Negative_Pass := True;
            Ada.Text_IO.Put_Line ("INFO negative_case=PASS expected=OCLW_SIGNATURE_INVALID");
         else
            Ada.Text_IO.Put_Line
              ("INFO negative_case=FAIL expected="
